...
package com.monetization.ikadplugin.ads.shimmer_effect;
...
public final class ShimmerDrawable extends Drawable {
  ...
  public void draw(@NonNull Canvas canvas) {
    ...
    if (mShimmer.direction == Shimmer.Direction.RIGHT_TO_LEFT) {
      dx = offset(translateWidth, -translateWidth, animatedValue);
      dy = 0f;
    } else if (mShimmer.direction == Shimmer.Direction.TOP_TO_BOTTOM) {
      dx = 0f;
      dy = offset(-translateHeight, translateHeight, animatedValue);
    } else if (mShimmer.direction == Shimmer.Direction.BOTTOM_TO_TOP) {
      dx = 0f;
      dy = offset(translateHeight, -translateHeight, animatedValue);
    } else {
      dx = offset(-translateWidth, translateWidth, animatedValue);
  ...
  private void updateShader() {
    ...
    final Shader shader;
    if (mShimmer.shape == Shimmer.Shape.RADIAL) {
      shader =
              new RadialGradient(
                      width / 2f,
                      height / 2f,
